...
exports.getBidsByJobId = async (req, res) => {
    try {
        ...
        const bidType = req.query.bidType;
        let matchQuery = {
...
exports.assignWorkerBid = async (req, res) => {
    try {
        const bid = await Bids.findById(req.body.bidId).exec();
        if (!bid) {
            return res.status(404).json({ message: 'Bid not found' });
        }

        if (req.user.role == 'manager') {
            const updatedBid = await Bids.findByIdAndUpdate(req.body.bidId, { status: 'approved' }).exec();
            const updatedJob = await Jobs.findByIdAndUpdate(req.body.jobId, { assignedWorkerBid: req.body.bidId, assignedTo: req.body.workerId, status: 'confirmed' }).exec();
            res.status(200).json({ message: 'Bid updated successfully', data: { bid: updatedBid, updatedJob } });
        } else {
            res.status(403).json({ message: 'Not authorized to update this bid' });
        }
    } catch (error) {
...
exports.assignVendorBid = async (req, res) => {
    try {
        const bid = await Bids.findById(req.body.bidId).exec();
        if (!bid) {
            return res.status(404).json({ message: 'Bid not found' });
        ...
        if (req.user.role == 'manager') {
            const updatedBid = await Bids.findByIdAndUpdate(req.body.bidId, { status: 'approved' }).exec();
            const updatedJob = await Jobs.findByIdAndUpdate(req.body.jobId, { jobVendor: req.body.vendorId, assignedVendorBid: req.body.bidId, status: 'confirmed' }).exec();
            res.status(200).json({ message: 'Bid updated successfully', data: { bid: updatedBid, updatedJob } });
        } else {
            res.status(403).json({ message: 'Not authorized to update this bid' });
        }
